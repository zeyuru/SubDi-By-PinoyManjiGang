...
class Mailer {
    ...
    private static function log(string $message): void {
        ...
        error_log($logMessage . PHP_EOL, 3, __DIR__ . '/mailer_debug.log');
    ...
    public static function send(string $toEmail, string $toName, string $subject, string $htmlBody): bool {
        ...
        if (file_exists($composerAutoload)) {
            ...
            if (class_exists('PHPMailer\PHPMailer\PHPMailer')) {
                self::log("PHPMailer available via Composer. Host={$cfg['host']}, Port={$cfg['port']}, User={$cfg['username']}");
                return self::sendViaPHPMailer($cfg, $toEmail, $toName, $subject, $htmlBody);
            }
            self::log('Composer autoload found, but PHPMailer class is missing. Falling back to native mail().');
        } else {
            self::log('PHPMailer not installed. Falling back to native mail().');
        }
        ...
        self::log("Native mail() send succeeded to {$toEmail}.");
        return true;
    }
    ...
    private static function sendViaPHPMailer(array $cfg, string $toEmail, string $toName, string $subject, string $htmlBody): bool {
        ...
        self::log("Sending email to {$toEmail} via PHPMailer (host={$cfg['host']}, port={$cfg['port']}).");
        $mail->send();
        self::log("PHPMailer send succeeded to {$toEmail}.");
        return true;
    }
